...
void select_indices(LabelT* data, unsigned data_size, std::vector<IndexT>& indices, Cond cond = Cond()) {
  ...
  */
  ...
  unsigned nb_test_obs;
  {
    ...
    // Load and train on 0s and 1s
    {
      ...
      // Load train dataset and labels on host. The dataset will not be transposed and the data dimension will be
      // padded  to a power of 2.
      auto host_train_data = read_mnist_images<DataT>(mnist_get_train_images_path(mnist_path),
                                                      obs_size, padded_obs_size, nb_train_obs,
                                                      false, true, normalize_factor);
      auto host_train_labels = read_mnist_labels<LabelT>(mnist_get_train_labels_path(mnist_path), nb_train_obs);

      // Copy dataset and labels to device
      ml::matrix_t<DataT> sycl_train_data(host_train_data, cl::sycl::range<2>(nb_train_obs, padded_obs_size));
      sycl_train_data.data_range[1] = obs_size;  // Specify the real size of an observation
      sycl_train_data.set_final_data(nullptr);
      ml::vector_t<LabelT> sycl_train_labels(host_train_labels, cl::sycl::range<1>(nb_train_obs));
      sycl_train_labels.set_final_data(nullptr);

      // Select 0s and 1s
      std::vector<unsigned> indices;
      select_indices(host_train_labels.get(), nb_train_obs, indices, select_labels);
      ...
      // Compute a new basis and apply it. The basis will be automatically loaded if already saved before,
      // otherise it will be computed and saved.
      split_train_data = apply_pca.compute_and_apply(q, split_train_data, pca_args);
    ...
    // Load and test on 0s and 1s
    {
      ...
      // Load test dataset and labels on host. The dataset will not be transposed and the data dimension will be
      // padded to a power of 2.
      auto host_test_data = read_mnist_images<DataT>(mnist_get_test_images_path(mnist_path),
                                                     obs_size, padded_obs_size, nb_test_obs,
                                                     false, true, normalize_factor);
      auto host_expected_test_labels = read_mnist_labels<LabelT>(mnist_get_test_labels_path(mnist_path), nb_test_obs);

      // Copy labels on device
      ml::matrix_t<DataT> sycl_test_data(host_test_data, cl::sycl::range<2>(nb_test_obs, padded_obs_size));
      sycl_test_data.data_range[1] = obs_size;  // Specify the real size of an observation
      sycl_test_data.set_final_data(nullptr);

      // Select 0s and 1s
      std::vector<unsigned> indices;
      select_indices(host_expected_test_labels.get(), nb_test_obs, indices, select_labels);
      auto split_test_data = split_by_index(q, sycl_test_data, indices);
      nb_test_obs = indices.size();
      host_split_expected_test_labels = ml::make_shared_array(new LabelT[nb_test_obs]);
      for (unsigned i = 0; i < nb_test_obs; ++i)
        host_split_expected_test_labels.get()[i] = host_expected_test_labels.get()[indices[i]];
  ...
  svm.print_score(host_split_predicted_test_labels.get(), host_split_expected_test_labels.get(), nb_test_obs);
}

 ...
 */
template <class T>
void eye(queue& q, matrix_t<T>& mat) {
  mat.assert_square();
 ...
 */
template <class T>
void transpose(queue& q, matrix_t<T>& in, matrix_t<T>& out) {
...
void avg(queue&, matrix_t<T>& dataset, vector_t<T>& avg) {
  ...
  static constexpr std::array<eig_index_t, 1> dims { D };
...
inline void center_data(queue& q, matrix_t<T>& dataset, vector_t<T>& data_avg) {
  mat_vec_apply_op<D>(q, dataset, data_avg, std::minus<T>());
}

...
void vec_unary_op(queue& q, sycl_vec_t<T>& in, sycl_vec_t<T>& out, UnaryOp op = UnaryOp()) {
  sycl::sycl_execution_policy<NameGen<0, ml_vec_unary_op, T, UnaryOp>> sycl_policy(q);
 ...
 */
template <class UnaryOp, class T>
inline void vec_unary_op(queue& q, sycl_vec_t<T>& in_out, UnaryOp op = UnaryOp()) {
...
void vec_binary_op(queue& q, sycl_vec_t<T>& in1, sycl_vec_t<T>& in2, sycl_vec_t<T>& out, BinaryOp op = BinaryOp()) {
  sycl::sycl_execution_policy<NameGen<0, ml_vec_binary_op, T, BinaryOp>> sycl_policy(q);
  transform(sycl_policy, begin(in1), end(in1), begin(in2), begin(out), op);
}

/**
 * @see vec_binary_op(queue&, sycl_vec_t<T>&, sycl_vec_t<T>&, sycl_vec_t<T>&, BinaryOp)
 * @tparam BinaryOp T -> T -> T
 * @tparam T
 * @param q
 ...
 */
template <class BinaryOp, class T>
inline void vec_binary_op(queue& q, sycl_vec_t<T>& in_out1, sycl_vec_t<T>& in2, BinaryOp op = BinaryOp()) {
 ...
 */
template <class BinaryOp, class T>
void sycl_transform(queue& q, sycl_vec_t<T>& v, T cst, BinaryOp op = BinaryOp()) {
  sycl::sycl_execution_policy<NameGen<0, ml_transform, T, BinaryOp>> sycl_policy(q);
  transform(sycl_policy, begin(v), end(v), begin(v), [=](T val) { return op(val, cst); });
}
...
T sycl_dist2(queue& q, sycl_vec_t<T>& v1, sycl_vec_t<T>& v2) {
  ...
  return inner_product(sycl_policy, begin(v1), end(v1), begin(v2), 0.0f, std::plus<T>(), functors::prod_diff<T>());
}
...
T sycl_dist2_opposite(queue& q, sycl_vec_t<T>& v1, sycl_vec_t<T>& v2) {
  ...
  return inner_product(sycl_policy, begin(v1), end(v1), begin(v2), 0.0f, std::plus<T>(), functors::prod_sum<T>());
}
 ...
 *
 * This is useful when comparing eigenvectors which direction does not matter for instance.
 *
 * @tparam T
 * @param q
 * @param v1
 ...
 */
template <class T>
inline T sycl_dist_no_direction(queue& q, sycl_vec_t<T>& v1, sycl_vec_t<T>& v2) {
  return std::sqrt(std::min(sycl_dist2(q, v1, v2), sycl_dist2_opposite(q, v1, v2)));
}
...
T sycl_min(queue& q, sycl_vec_t<T>& v) {
  ...
  return reduce(sycl_policy, begin(v), end(v), std::numeric_limits<T>::max(), [](T a, T b) { return a < b ? a : b; });
}
...
T sycl_max(queue& q, sycl_vec_t<T>& v) {
  ...
  return reduce(sycl_policy, begin(v), end(v), std::numeric_limits<T>::min(), [](T a, T b) { return a > b ? a : b; });
}

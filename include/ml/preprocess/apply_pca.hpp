...
/**
 * @brief Helper to compute and apply the PCA from a training set and applying it on a test set.
 *
 * @see pca_svd
 * @tparam T
 */
...
class apply_pca {
public:
  apply_pca() : _enable_pca(true), _nb_vec_computed(0), _data_avg(range<1>()), _eigenvectors(range<2>()) {}

  /**
   * @brief Either load the eigenvectors or compute them and apply the PCA to the given data.
   *
   * @param q
   ...
   * @return the new data
   */
  matrix_t<T> compute_and_apply(queue& q, matrix_t<T>& data, const pca_args<T>& pca_args) {
    ...
    _data_avg = vector_t<T>(range<1>(data_dim), get_optimal_nd_range(data_dim_pow2));

    std::string load_filename = get_filename(data_dim_pow2, pca_args.min_nb_vecs, pca_args.scale_factor);
    if (pca_args.auto_load && file_exists(load_filename)) {
      ...
      _eigenvectors = matrix_t<T>(range<2>(pca_args.min_nb_vecs, data_dim),
      ...
      if (pca_args.save)
        save_array(q, _eigenvectors, get_filename(data_dim_pow2, _nb_vec_computed, pca_args.scale_factor));
    }

    matrix_t<T> new_data = matrix_t<T>(range<2>(access_data_dim(data, 0), _nb_vec_computed));
    mat_mul<LIN, TR>(q, data, _eigenvectors, new_data);
    return new_data;
  }

  /**
   * @brief Apply the PCA to a dataset from previously computed eigenvectors and data_avg.
   *
   * @param q
   * @param[in, out] data this matrix has been centered after this call
   * @return the new data
...
private:
   ...
   * @return the filename
   */
  inline std::string get_filename(SYCLIndexT data_dim_pow2, SYCLIndexT nb_vec, T svd_factor) {
    std::stringstream ss;
    ss << "eig_svd_" << nb_vec << "_" << data_dim_pow2 << "_" << svd_factor << "_" << typeid(T).name();
    return ss.str();
  }
